...
interface BlogRatingScaleProps {
    blogId: string;
    initialRating?: number;
}

const BlogRatingScale: FC<BlogRatingScaleProps> = ({ blogId, initialRating = 0 }) => {
    const [rating, setRating] = useState(initialRating);
    const { patch, isLoading } = useAPI(`${ENDPOINTS.blogs}/${blogId}`);
    ...
    return (
        <Box display='flex' direction='col' justifyContent='center' alignItems='center' className="blog-rating-scale mt-8 p-6 rounded-2xl bg-white/5 border border-white/10">
...
export default BlogRatingScale;
