...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ onMenuToggle }) => {
    ...
    return (
        <>
            {showProfileModal && (
                <ProfileForm onClose={() => setShowProfileModal(false)} />
            )}
            ...
            <header className="fixed top-0 right-0 left-0 h-20 bg-secondary/5 backdrop-blur-md border-b border-secondary/10 flex items-center justify-between px-4 md:px-8 z-50 transition-all">
                ...
                <div className="flex items-center gap-3 md:gap-6">
                    ...
                    <div className="relative">
                        <div
                            className="flex items-center gap-2 md:gap-3 bg-white/5 p-1.5 md:p-2 rounded-full border border-white/10 hover:bg-white/10 transition-all cursor-pointer"
                            onClick={handleToggleUserMenu}
                        >
                            <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-secondary/20 flex items-center justify-center text-secondary font-bold uppercase text-xs md:text-sm overflow-hidden border border-white/10">
                            ...
                            <i className={`bi bi-chevron-down text-[10px] md:text-xs text-light/40 ml-1 md:ml-2 transition-transform duration-300 ${showUserMenu ? 'rotate-180' : ''}`}></i>
                        </div>
                            ...
                            {showUserMenu && (
                                <>
                                    ...
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, y: 10, scale: 0.95 }}
                                        transition={{ duration: 0.2 }}
                                        className="absolute left-0 mt-4 w-56 bg-primary border border-primary/10 rounded-2xl shadow-2xl overflow-hidden z-50 text-right"
                                    >
                                        <div className="p-4 border-b border-white/5 bg-secondary/5 flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-secondary/20 flex items-center justify-center text-secondary font-bold uppercase overflow-hidden shrink-0 border border-white/10">
                                                {user?.image && user.image.startsWith('http') ? (
                                                    <img src={user.image} alt={user.name} className="w-full h-full object-cover" />
                                                ) : (
                                                    user?.name?.charAt(0) || 'A'
                                            ...
                                            <div className="overflow-hidden">
                                                <Typography variant="p" size="body2" color="white" className="font-bold line-clamp-1">{user?.name || 'المدير'}</Typography>
                                                <Typography variant="p" size="body1" color="light" className="opacity-40 line-clamp-1 text-[10px]">{user?.email || 'admin@nawjha.tech'}</Typography>
                                            </div>
                                        </div>
                                        <div className="py-2">
                                            <button
                                                ...
                                                className="flex items-center justify-end gap-3 w-full px-4 py-2.5 text-xs text-light/80 hover:bg-white/5 hover:text-secondary transition-all"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="flex items-center justify-end gap-3 w-full px-4 py-2.5 text-xs text-red-400 hover:bg-red-500/10 transition-all font-medium"
                                            >
                                                تسجيل الخروج
